...
exports.AcceptRequestnotification = async (req, res) => {
  const { id, nid, user_id } = req.body;
  const wss = req.wss;
  ...
  try {
    ...
    db.query(
      `UPDATE friendRequest_accept SET status = 'Yes' WHERE id = ?`,
      [id],
      (updateErr, updateResults) => {
        if (updateErr) {
          return res
        ...
        db.query(
          `SELECT u.*, fr.id AS frq_id, fr.user_id AS sentid
           FROM users u
           JOIN friendRequest_accept fr ON u.id = fr.user_id
           WHERE fr.sent_to = ? AND fr.status = ?`,
          [updateResults.insertId, "No"], // insertId likely doesn't work here
          (queryErr, results) => {
            if (queryErr) {
              return res
                .status(500)
            ...
            db.query(query, [user_id, user_id], (err, friendResults) => {
              if (err) {
                return res
                  .status(500)
                  .json({ message: "Database query error", error: err });
              ...
              db.query(
                ...
                (err, userResult) => {
                  ...
                  const userUsername =
                    userResult[0]?.username || "Unknown User";
                  ...
                  db.query(
                    ...
                    (err, senderResult) => {
                      ...
                      const broadcastMessage = JSON.stringify({
                        event: "friendrequestacceptnotification",
                        user_id: friendResults,
                        LoginData: friendResults,
                      });
                      ...
                      db.query(
                        ...
                        (err, notifyResult) => {
                          if (err) {
                            ...
                            // Do not send response again, just log
                          } else {
                            db.query(
                              `INSERT INTO notification (to_id, user_id, message, date) VALUES (?, ?, ?, ?)`,
                              [
                                user_id,
                                notifyResult[0].to_id,
                                notificationMessage,
                                date,
                              ],
                              (err) => {
...
exports.getReceivedMessageheaderTotal = async (req, res) => {
  const user_id = req.body.user_id;
...
const countImagesInUserAlbum = () => {
  return new Promise((resolve, reject) => {
    const query = "SELECT images FROM usersalbum";
    db.query(query, (err, results) => {
      ...
      results.forEach((row) => {
        try {
          ...
          const imageCount = imagesArray.filter((url) =>
            url.toLowerCase().match(/\.(jpg|jpeg|png|gif|webp|svg)$/)
          ).length;
  ...
  try {
    const mainQuery = `
      SELECT COUNT(*) AS total_images FROM (
        SELECT image AS file FROM gallery WHERE LOWER(image) REGEXP '\\.(jpg|jpeg|png|gif|webp|svg)$'
        UNION ALL
        SELECT image AS file FROM \`groups\` WHERE LOWER(image) REGEXP '\\.(jpg|jpeg|png|gif|webp|svg)$'
        UNION ALL
        SELECT file FROM group_post WHERE LOWER(file) REGEXP '\\.(jpg|jpeg|png|gif|webp|svg)$'
        UNION ALL
        SELECT image AS file FROM events WHERE LOWER(image) REGEXP '\\.(jpg|jpeg|png|gif|webp|svg)$'
        UNION ALL
        SELECT file FROM event_post WHERE LOWER(file) REGEXP '\\.(jpg|jpeg|png|gif|webp|svg)$'
        UNION ALL
        SELECT image AS file FROM forum WHERE LOWER(image) REGEXP '\\.(jpg|jpeg|png|gif|webp|svg)$'
        UNION ALL
        SELECT file FROM chatmessages WHERE LOWER(file) REGEXP '\\.(jpg|jpeg|png|gif|webp|svg)$'
      ) AS all_images;
    `;

    db.query(mainQuery, async (queryErr, results) => {
      if (queryErr) {
        return res.status(500).json({
          message: "Database query error",
          error: queryErr,
      ...
      const userAlbumImagesCount = await countImagesInUserAlbum();

      const totalImages =
        (results[0]?.total_images || 0) + userAlbumImagesCount;
...
exports.getUserAlbumImageCountDashboard = async (req, res) => {
  try {
    db.query(`SELECT images FROM usersalbum`, (err, results) => {
      if (err) {
      ...
      results.forEach((row) => {
        try {
          const urls = JSON.parse(row.images); // parse the JSON string
          if (Array.isArray(urls)) {
            const imageCount = urls.filter(
              (url) =>
                typeof url === "string" &&
                url.toLowerCase().match(/\.(jpg|jpeg|png|gif|webp|svg|avif)$/)
            ).length;
