  ...
  try {
    ...
    db.query(
      ...
      async (err, rows) => {
        ...
        const query = `
          INSERT INTO users (looking_for,attempt_count,IpAddress,sexuality,status,profile_image,security_question,security_answer,viewpassword, slug, username, email, password, birthday_date, profile_type, preferences, gender,created_at) 
          VALUES (?,?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?,?,?)`;
...
exports.updateProfile = async (req, res) => {
  ...
  var mainherData = req.body;
  console.log(mainherData);
    ...
    async (err, result) => {
      ...
      try {
        ...
        function updateUserProfile() {
          ...
          if (genders === "Couple") {
            db.query(
              `UPDATE users SET profile_type=?,subregion=?,sexuality=?,
              ...
              profile_image=?, slug=?,preferences=?,interstedin_her=?,relationship_status_her=?,search_looking_for_her=?,drinker_her=?,smoker_her=?,degree_her=?,	nationality_her=?,bodytype_her=?,height_feet_her=?,sexual_orientation_her=?,connectwith_her=?,tattos_her=?,	body_piercings_her=?,fetish_her=?
              WHERE email=?`,
              [
                "Couples profile",
                subregion,
                ...
                slug,
                preferences,
                mainherData.interstedin_her,
                mainherData.relationship_status_her,
                mainherData.search_looking_for_her,
                mainherData.drinker_her,
                mainherData.smoker_her,
                mainherData.degree_her,
                mainherData.nationality_her,
                mainherData.bodytype_her,
                mainherData.height_feet_her,
                mainherData.sexual_orientation_her,
                mainherData.connectwith_her,
                mainherData.tattos_her,
                mainherData.body_piercings_her,
                mainherData.fetish_her,
                email,
              ],
              (err, result) => {
                if (err) {
                ...
                if (Number(makeImagePrivate) === 0) {
                  ...
                  if (req.files["profile_image"]?.[0]?.location === undefined) {
                  } else {
                    Createnewgallery();
                  }
                  // Create gallery entry after user profile is updated
                }
              }
            );
          } else {
            db.query(
              ...
              bodytype=?, height_feet=?, height_inches=?, sexual_orientation=?, relationship_status=?,
              search_looking_for=?, degree=?, drinker=?, smoker=?, tattos=?, body_piercings=?, fetish=?,
              profile_image=?, slug=?,preferences=?
              WHERE email=?`,
              [
                "Single profile",
                subregion,
...
FROM userphotoprivate up
JOIN users u ON u.id = up.user_id
LEFT JOIN albums a ON a.id = up.albumid 
WHERE up.to_id = ? 
ORDER BY up.id DESC;
;
`,
  ...
  db.query(
    ...
    async (err, rows) => {
      ...
      if (rows.length > 0) {
        ...
        // If question & answer are provided, validate them
        if (question && securityAnswer) {
          ...
          // If no question & answer provided, return stored password
          return res.status(200).json({
            message: "Password retrieved successfully,Please check your email",
