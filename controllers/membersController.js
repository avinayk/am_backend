  ...
  try {
    ...
    db.query(userQuery, [user_id], (err, userResults) => {
      ...
      if (likedGenderArray.length > 0) {
        ...
        sql += `gender IN (${genderPlaceholders}) AND (`;
        params.push(...likedGenderArray);
      } else {
        sql += `(`; // Just start the preference condition if no gender filter
      }
      ...
      if (prefKeywords.length > 0) {
        const keywordPlaceholders = prefKeywords
          ...
          .join(" OR ");
        sql += `preferences IS NULL OR preferences = '' OR ${keywordPlaceholders}`;
        params.push(...prefKeywords);
      } else {
        sql += `preferences IS NULL OR preferences = ''`;
      }
...
exports.getuserChatmessage = async (req, res) => {
  ...
  try {
    ...
    const query = `
      ...
      ORDER BY  cm.date desc limit 10 OFFSET ?;
    `;

    // Fetching the messages
    db.query(
      query,
      [user_id, user_id, to_id, to_id, user_id, user_id, parseInt(offset)],
      (err, results) => {
        if (err) {
          return res.status(500).json({
            message: "Database query error",
...
exports.getAllgallery = async (req, res) => {
  ...
  try {
    ...
    const query = `
          SELECT g.*, u.username, u.profile_type, u.gender,u.birthday_date,u.location
          FROM gallery g
...
exports.getGalleryGroupforumDetail = async (req, res) => {
  ...
  try {
    ...
    if (post_type === "gallery") {
      query = `
        SELECT g.*, 'gallery' AS pertype, u.username, u.id AS uid, u.profile_type, u.gender, u.profile_image,
          COUNT(gf.id) AS total_favourites,
          CASE
            WHEN EXISTS (
...
GROUP BY 
  gp.id, u.username, u.id, u.profile_type, u.gender, u.profile_image;
      ...
      query = `
       SELECT f.*, 'forum' AS pertype, u.username, u.id AS uid, u.profile_type, u.gender, u.profile_image,
        ...
        CASE
          ...
          ELSE 0
        END AS user_favourited  -- Checking if the current user has favourited the post
      FROM forum f
...
exports.searchfilter = async (req, res) => {
  ...
  try {
    ...
    const query = `
    SELECT g.*, u.username, u.gender, u.female, u.male, u.couple,u.birthday_date,u.location
      FROM gallery g
      JOIN users u ON g.user_id = u.id
        ...
        if (selectResults.length > 0) {
          ...
          db.query(
            ...
            (insertErr, result) => {
              ...
              const query = "SELECT name FROM albums WHERE id = ?";
              db.query(query, [albumid], (err, results) => {
                ...
                if (results.length > 0) {
                  const albumName = results[0].name;
                  const message = `Request to view album: ${albumName}`;
...
`;
    ...
    // Combine userIdsArray and offset as query parameters
    const queryParams = [
...
exports.membersearch = async (req, res) => {
  const { user_id, search } = req.body;
  let lookinfor = req.body.looking_forr;
  ...
  try {
    ...
    db.query(userQuery, [user_id], (err, userResults) => {
      ...
      if (likedGenderArray.length > 0) {
        ...
        LOWER(town) LIKE LOWER(?) OR
        LOWER(birthday_date) LIKE LOWER(?) OR
        LOWER(username) LIKE LOWER(?) OR
        LOWER(nationality) LIKE LOWER(?) OR
        LOWER(relationship_status) LIKE LOWER(?) OR
        LOWER(degree) LIKE LOWER(?) OR
        LOWER(drinker) LIKE LOWER(?) OR
        LOWER(smoker) LIKE LOWER(?) OR
        LOWER(tattos) LIKE LOWER(?) OR
        LOWER(body_piercings) LIKE LOWER(?) OR
        LOWER(fetish) LIKE LOWER(?) OR
        LOWER(connectwith) LIKE LOWER(?))
      `;

        // Fill 14 fields with LOWER('%searchTerm%')
        const paramss = Array(14).fill(`%${searchTerm.toLowerCase()}%`);
        // 14 fields for conditions, filled with searchTerm
        ...
        if (lookinfor && Array.isArray(lookinfor) && lookinfor.length > 0) {
          ...
          paramss.push(...lookinfor); // Add the actual values for lookinfor
        }
      ...
      `;
        ...
        const sql = `
          SELECT * FROM users u
          WHERE (preferences IS NULL OR preferences = '' OR ${keywordPlaceholders})
            AND ${conditions}
        `;
  ...
  try {
    ...
    if (
      ...
      db.query(userQuery, [user_id], (err, userResults) => {
        ...
        if (prefKeywords.length > 0) {
          const keywordPlaceholders = prefKeywords
            .map(() => "LOWER(u.preferences) LIKE ?")
            .join(" OR ");
          conditions.push(
            `(u.preferences IS NULL OR u.preferences = '' OR ${keywordPlaceholders})`
          );
          params.push(...prefKeywords);
        } else {
          conditions.push(`(u.preferences IS NULL OR u.preferences = '')`);
        }
