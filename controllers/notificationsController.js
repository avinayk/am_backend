...
exports.getnotificationsall = async (req, res) => {
  ...
  try {
    ...
    const unreadNotificationsQuery = `SELECT COUNT(*) AS total FROM notification WHERE user_id = ? AND \`read\` = 'No'`;

    // Query to count unread chat messages
    const unreadChatsQuery = `SELECT COUNT(*) AS total FROM chatmessages WHERE to_id = ? AND \`read\` = 'No'`;
    ...
    db.query(query, [user_id], (err, notifications) => {
      ...
      db.query(
        ...
        (err, notificationCountResult) => {
          ...
          db.query(unreadChatsQuery, [user_id], (err, chatCountResult) => {
            if (err) {
              return res
                .status(500)
                .json({ message: "Error counting chat messages", error: err });
            }

            // Calculate total unread count
            const unreadNotifications = notificationCountResult[0].total;
            const unreadChats = chatCountResult[0].total;
            const totalUnread = unreadNotifications + unreadChats;

            return res.status(200).json({
              results: notifications,
              resultcount: totalUnread, // Total unread count
            });
          });
        }
      );
